...
class CA2D(Automaton):
    ...
    def __init__(self, size, s_num='23', b_num='3', random=False, device = "cpu"):
        ...
        self.world = torch.zeros((self.h,self.w),dtype=torch.int).to(self.device)
        self.reset()
    ...
    def reset(self):
        ...
        if(self.random):
            ...
            self.world = torch.zeros_like(self.world,dtype=torch.int).to(self.device)
            self.world[self.w//2-1:self.w//2+1,self.h//2-1:self.h//2+1]=torch.ranint(0,2,(2,2)).to(self.device)
    ...
    def get_init_mat(self,rand_portion):
        ...
        """
        batched_size = torch.tensor([self.h,self.w]).to(self.device)
        randsize = (batched_size*rand_portion).to(dtype=torch.int16).to(self.device) # size of the random square
        randstarts = (batched_size*(1-rand_portion)/2).to(dtype=torch.int16).to(self.device) # Where to start the index for the random square

        randsquare = torch.where(torch.randn(*randsize.tolist())>0,1,0) # Creates random square of 0s and 1s

        init_mat = torch.zeros((self.h,self.w),dtype=torch.int16).to(self.device)
