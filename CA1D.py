...
class CA1D(Automaton):
    ...
    def step(self):
        ...
        self.world=self.rule[indices.long()] # This is the same as [ rule[indices[i]] for i in range(W)]
...
class GeneralCA1D(Automaton):
    ...
    def draw(self):
        self._worldmap[:,self.time%self.h,:]=self.colors[self.world.long(),:].permute(1,0) # (3,W), contains the RGB values of the colors
    ...
    def step(self):
        ...
        self.world=self.rule[summed_states.long()]
